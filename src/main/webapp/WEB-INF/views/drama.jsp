		...
		<div class="row" style="float: none; margin: 100 auto">
			<div>
				...
				<div class="d-flex justify-content-between mb-3">
						...
						<nav aria-label="Page navigation example"
							...
							<ul class="pagination">
								...
								<c:choose>
									<c:when test="${page.get(0) > 5 && search eq 'search' }">
										<li class="page-item" style="margin: 0; padding: 0;"><a
											class="page-link"
											href="/board/list/40/args/${page.get(0)-1}?title=${title}"
											style="color: black;">이전</a></li>
									</c:when>
									<c:when test="${page.get(0) > 5 && search != 'search' }">
										<li class="page-item" style="margin: 0; padding: 0;"><a
											class="page-link" href="/board/list/40/${page.get(0)-1}"
											style="color: black;">이전</a></li>
									</c:when>
								</c:choose>
								...
								<c:forEach var="pg" varStatus="pages" items="${page}">
									<li class="page-item"
										style="margin: 0; padding: 0; color: black;"><c:choose>
											<c:when test="${search eq 'search'}">
												<c:if test="${selectpage == pg}">
													<a class="page-link bg-dark text-white"
														href="/board/list/40/args/${pg}?title=${title}"> ${pg} </a>
												</c:if>
												<c:if test="${selectpage != pg}">
								...
								<c:choose>
									<c:when test="${page.size() == 5 && search eq 'search' }">
										<li class="page-item" style="margin: 0; padding: 0"><a
											class="page-link"
											href="/board/list/40/args/${page.get(page.size()-1)+1}"
											style="color: black;">다음</a></li>
									</c:when>
									<c:when test="${page.size() == 5 && search != 'search'}">
										<li class="page-item" style="margin: 0; padding: 0"><a
											class="page-link"
											href="/board/list/40/${page.get(page.size()-1)+1}"
											style="color: black;">다음</a></li>
									</c:when>
								</c:choose>
					...
					<div style="display: inline-block">
						<form action="/board/list/40/args/1" method="get">
